...
> Historical note: v0.3 shipped Git worktree scanning and release hardening.
> Streaming APIs and broader config parity were deferred to the backlog.
...
v0.2 intentionally modeled the highest-value config fields first. This work was
completed in v0.4 after reviewing upstream `docs/config.md` and the Betterleaks
`v1.6.1` config package.

Resolved in v0.4:

- Modern Expr filter helpers replaced legacy allowlist modeling.
- `Rule.entropy` was added only as a compatibility field.
- Validation result helpers were added for common `validate` expressions.
- Common rule constructors were added.
- Relative `extend.path` behavior for inline configs was documented and
  implemented.
- No archive/decode/file-size tuning fields were added because they are not
  stable config fields in the reviewed Betterleaks surface.

Acceptance criteria kept for future upstream config additions:
